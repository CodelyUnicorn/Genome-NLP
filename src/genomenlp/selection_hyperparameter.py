...
def plot_eval_results(eval_results):
    ...
    eval_df = pd.DataFrame(list(eval_results.items()), columns=['Metric', 'Value'])
    
    # Set up the matplotlib figure
    plt.figure(figsize=(10, 6))
    
    # Create a barplot of the evaluation metrics
    sns.barplot(x='Metric', y='Value', data=eval_df)
...
def optuna_hp_space(trial):
    return {
        ...
        "logging_steps": trial.suggest_int("logging_steps", 10, 500),
    }
...
def main():
    ...
    #                     help='path to [ csv | csv.gz | json | parquet ] file')3
    ...
    train = 'data1/train.csv'
    format = 'csv'
    model = args.model
    device = args.device
    test = "data1/test.csv"
    valid = 'data1/valid.csv'
    tokeniser_path = 'data/tokens.json'
    hyperparameter_file = ""
    vocab_size = args.vocab_size
    shuffle = args.no_shuffle
    wandb_state = 'OFF'
    ...
    for i in dataset:
        ...
        # print(type(dataset[i].features[args.label_names[0]]))
        if type(dataset[i].features['labels']) != ClassLabel:
    ...
    if model == "distilbert":
        ...
        # model = DistilBertForSequenceClassification(config) #// loading my pre-train model.
        
        
        # Define fine-tuning model
        model = DistilBertForSequenceClassification.from_pretrained("Out/model_files", num_labels=2)
    ...
    # eval_labels = list(eval_df["OUTPUT_LABEL"]) # replace "LABEL_COLUMN" with the actual name of your label column
    ...
    print("Best Run Hyperparameter search")
