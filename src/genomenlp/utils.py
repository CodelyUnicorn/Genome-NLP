...
def _compute_metrics(eval_preds):
    ...
    preds = np.argmax(logits, axis=-1) #############Commented due to error 'dataset not define and wandb'###################
...
def get_feature_importance_per(clf, x_test, y_test, features, model_type,
    ...
    plt.close()
    return per_scores
###################### change the code bit, applied generator ####################
def build_kmers(sequence: str, ksize: int) -> Generator[str, None, None]:
...
class _EmbedSeqsKmers(object): ################# Commented as this class not accessed and error was' chuck and EmbedSeqsKmers not defined'######### 
